...
int state_init(tfs_params params) {
    ...
    pthread_rwlock_init(&freeinode_ts_lock, NULL);
...
int state_destroy(void) {
    ...
    pthread_rwlock_destroy(&freeinode_ts_lock);
...
int inode_create(inode_type i_type) {
    ...
    case T_FILE:
        ...
        inode_table[inumber].i_hard_link_count = 1;
        inode_table[inumber].origin_path[0] = '\0';
        break;
    default:
...
int is_in_open_file_table(int inumber) {
    ...
    for (i = 0; i < MAX_OPEN_FILES; i++) {
        if (inumber == open_file_table[i].of_inumber &&
            free_open_file_entries[i] == TAKEN)
            return true;
    }
